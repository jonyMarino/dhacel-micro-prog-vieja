...
/*esta funcion es igual que la anterior pero graba siempre, aunque el dato sea igual  */
 byte EscribirWordSiempre(word Addr,int valor){
  ...
  if (Addr<FLASH_PARAMETROS_START)
    return ERR_RANGE;
  #ifdef jony_22_08
  if(1){  // no importa si el valor el el mismo
    byte shift;
...
/*
Se utiliza para escribir dentro del sector para salvar valores(FLASH_APAGADO_START)
*/
void WriteNWord(char cantidad,word *val){
  char i;
  for(i=0;i<cantidad;i++)
    WriteWord(FLASH_APAGADO_START+2*i,val[i]);    
