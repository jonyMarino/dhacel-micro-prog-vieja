...
int tol = pSegmento->tolerancia;
 if(tol){
    ...
    switch(pSegmento->tipo_tolerancia){
      case Lo: if((getValCalculado(canal)+tol)<=SetPointPrograma[canal])
                    return FALSE;
      				 break;
      case bAnd:if((getValCalculado(canal)+tol)<=SetPointPrograma[canal])
                    return FALSE;			 
      case Hi: if((getValCalculado(canal)-tol)>=SetPointPrograma[canal])
                    return FALSE;
    } 
  }
  return TRUE;
