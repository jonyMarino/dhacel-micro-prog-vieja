...
#endif
#include "Mmax.h"
//extern const int div_dec[]={1,10,100,1000,10000};
/////////////LEDS//////////////////////
 ...
 if (Mostrar_Proc){										                      // Se actualizaron los valores???
  ...
  for(i=0;i<CANTIDAD_CANALES;i++){							            // Recorrer todos los canales
    if (Result[i]==ERR_OK){  							                  // El REsultado de la linealización fue OK??
        ...
        #ifdef SENS_HUMEDAD
          if(i==0)                                    // canal 1 ? 
             Pasar_Numero(ValFinal[i],i,DotNum[i]);   //muestro la temp bs 
          if(i==1){                                   // canal 2 ?
            if(PRom[R_Sensor+1]==SENSOR_HUM)          //humedad ?
              Pasar_Numero(Humedad,i,DotNum[i]);      //muestro la humedad  
            else
              Pasar_Numero(ValFinal[i],i,DotNum[i]);  // si no, muestro la temp bh 
          }
        #else
      ...
      if((int *)DirPar!=&SetPoint[R_SetPoint+0])
        ...
        Pasar_Numero(*(int *)DirPar,1,DotNum[1]);               // Lo muestro en DisplayInf y chau
            ...
            Pasar_Numero(dutytmp,1,1);                  //presento el valor de la potencia 
              ...
              #elif defined (VF)
                 if(PRom[R_MVF]==_SP)
                   Pasar_Numero(SetPoint[0],1,DotNum[1]);                 // si nada de lo anterior,Mostrar Setpoint
              #elif defined (LUX) && defined (INTEGRADOR)
                   Pasar_Numero(ValFinal[0],1,DotNum[1]);
...
if(PRom[R_Decimales+chan] && SENSOR_Maximo(sensor)>9999){
   Lim_Sup_ch[chan]=9999;
}
